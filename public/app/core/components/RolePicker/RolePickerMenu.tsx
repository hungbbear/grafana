    ...
    >
      <div className={customStyles.menu} aria-label="Role picker menu">
        <CustomScrollbar autoHide={false} autoHeightMax={`${MENU_MAX_HEIGHT}px`} hideHorizontalTrack hideVerticalTrack>
          ...
          {Object.entries(rolesCollection).map(([groupType, roles]) => {
            return (
              <RoleMenuGroupsSection
                ...
                groupSelected={(group: string) => groupSelected(GroupType.fixed, group)}
                groupPartiallySelected={(group: string) => groupPartiallySelected(GroupType.fixed, group)}
                onChange={(group: string) => onGroupChange(GroupType.fixed, group)}
                onOpenSubMenuRMGS={(group: string) => onOpenSubMenu(GroupType.fixed, group)}
...
export const RoleMenuGroupsSection = React.forwardRef<HTMLDivElement, RoleMenuGroupsSectionProps>(
  (
    ...
    const renderedName = rolesType.charAt(0).toUpperCase() + rolesType.slice(1) + ' roles';
    return (
      <div>
        {roles.length > 0 && (
          <div className={customStyles.menuSection}>
            <div className={customStyles.groupHeader}>{renderedName}</div>
            <div className={styles.optionBody}></div>
            {showGroups && !!optionGroups[rolesType]?.length
              ? optionGroups[rolesType].map((option, i) => (
                  <RoleMenuGroupOption
                    data={option}
                    key={i}
